...
<div class="mb-2 p-2">
    <div class="row mb-3">
        <div class="flex-1 d-flex justify-content-between align-content-center">
            ...
            <div>
                <a href="{{route('arp.create')}}" class="d-flex align-items-center">
                    Cadastrar Nova ARP <i class="mdi mdi-arrow-right font-16 mr-1 ml-2"></i>
                </a>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="input-group">
            <input type="text" id="numeroPregao" class="form-control col-2" style="height: 60px !important;width: 100%;"
                   placeholder="Número" name="number" value="{{$filters['number'] ?? ''}}">
            <input type="text" id="descricao" class="form-control" style="height: 60px !important;width: 100%;"
                   placeholder="Objeto" name="object" value="{{$filters['object'] ?? ''}}"/>
            <input data-perm="fornecedor-master" type="text" id="comprador" class="form-control"
                   style="height: 60px !important;width: 100%;" placeholder="Comprador" name="organ" value="{{$filters['organ'] ?? ''}}"/>
            <select id="valorStatus" class="search form-control col-2" name="status"
...
<div class="row collapse card-filtros" style="padding: 0 1rem;" id="buscaAvancada">
    <div class="col row mt-3">
        <div class="col mb-2">
            ...
            <select id="criterio-julgamento" class="form-control custom-select" name="modality" data-placeholder="Julgamento"
        ...
        <div class="col data mb-2">
            ...
            <div class="input-group date form_datetime align-items-center">
                <input type="date" id="dataFinal" autocomplete="off" name="dataFinal"
                       ...
                       aria-invalid="false"
                       placeholder="Até:"
                       value="@if(isset($filters['intervalo_data']['dataFinal'])) {{\Carbon\Carbon::parse($filters['intervalo_data']['dataFinal'])->format('Y-m-d') }} @endif"
                       style="height: 50px;border-radius: 5px;"/>
            </div>
        </div>
    </div>
